...
- **UI:** `streamlit` — один процесс, запуск на ноутбуке через `streamlit run app/main.py`, никакого JS-инструментария. На фазе 5 (Hetzner) тот же UI работает за HTTPS reverse-proxy.
...
Локальный запуск: `uv run pytest`, `uv run streamlit run app/main.py`, `uv run python scripts/ingest_corpus.py`.
...
- `/home/anton/projects/tadf/app/main.py`
